...
import com.migueltcc.fertintelligence.model.fertintelligence.fertilizationTables.criteria.ExchangeableBaseRatioModel;
import com.migueltcc.fertintelligence.repository.ExchangeableBaseRatioRepository;
...
@Service
public class ExchangeableBaseRatioServiceImpl implements ExchangeableBaseRatioService {

    @Autowired
    private ExchangeableBaseRatioRepository exchangeableBaseRatioRepository;
    ...
    @Override
    @Transactional
    public ExchangeableBaseRatioResponseDto createExchangeableBaseRatio(Long tableId, ExchangeableBaseRatioCreateRequestDto createRequestDto, String username) {
        ...
        exchangeableBaseRatioRepository.findByTable(table).ifPresent(existing -> {
            throw new IllegalStateException("Já existe uma tabela de relações entre bases cadastrada para esta tabela.");
        });

        ExchangeableBaseRatioModel criterion = ExchangeableBaseRatioModel.builder()
                .table(table)
                .build();
        BeanUtils.copyProperties(createRequestDto, criterion);

        return exchangeableBaseRatioRepository.save(criterion).toDto();
    }

    @Override
    @Transactional(readOnly = true)
    public ExchangeableBaseRatioResponseDto getExchangeableBaseRatioById(Long criterionId, String username) {
        UserModel owner = findUserByUsernameOrThrow(username);
        ExchangeableBaseRatioModel criterion = findCriterionByIdOrThrow(criterionId);
    ...
    @Override
    @Transactional(readOnly = true)
    public ExchangeableBaseRatioResponseDto getExchangeableBaseRatioByTable(Long tableId, String username) {
        UserModel owner = findUserByUsernameOrThrow(username);
        SoilFertilityInterpretationCriteriaTableModel table = findTableByIdOrThrow(tableId);
        checkViewPermission(table, owner);

        ExchangeableBaseRatioModel criterion = exchangeableBaseRatioRepository.findByTable(table)
                .orElseThrow(() -> new EntityNotFoundException(
                        "Tabela de relações entre bases não encontrada para a tabela: " + tableId));
        return criterion.toDto();
    }

    @Override
    @Transactional
    public ExchangeableBaseRatioResponseDto updateExchangeableBaseRatio(Long criterionId, ExchangeableBaseRatioPostRequestDto updateRequestDto, String username) {
        UserModel owner = findUserByUsernameOrThrow(username);
        ExchangeableBaseRatioModel criterion = findCriterionByIdOrThrow(criterionId);
    ...
    @Override
    @Transactional
    public void deleteExchangeableBaseRatio(Long criterionId, String username) {
        UserModel owner = findUserByUsernameOrThrow(username);
        ExchangeableBaseRatioModel criterion = findCriterionByIdOrThrow(criterionId);
        checkModifyPermission(criterion.getTable(), owner);
        exchangeableBaseRatioRepository.delete(criterion);
    }
    ...
    private ExchangeableBaseRatioModel findCriterionByIdOrThrow(Long criterionId) {
        return exchangeableBaseRatioRepository.findById(criterionId)
                .orElseThrow(() -> new EntityNotFoundException(
                        "Tabela de relações entre bases não encontrada com o ID: " + criterionId));
    }
