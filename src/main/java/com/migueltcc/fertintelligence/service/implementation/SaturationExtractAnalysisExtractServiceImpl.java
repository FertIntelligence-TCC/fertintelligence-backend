...
import java.util.stream.Collectors;
    ...
    public class SaturationExtractAnalysisExtractServiceImpl implements SaturationExtractAnalysisExtractService {
        ...
        @Override
        @Transactional
        public SaturationExtractAnalysisExtractResponseDto createSaturationExtractAnalysisExtract(Long rangeExtractId,
                                                                                                  Long layerExtractId,
        ...
        public SaturationExtractAnalysisExtractResponseDto getSaturationExtractAnalysisExtractById(Long saturationExtractAnalysisExtractId,
                                                                                                   String username) {
            UserModel owner = findUserByUsernameOrThrow(username);
            checkUserIsProprietario(owner);

            SaturationExtractAnalysisExtractModel analysisExtract = findSaturationExtractAnalysisExtractByIdOrThrow(saturationExtractAnalysisExtractId);
        ...
        @Override
        @Transactional(readOnly = true)
        public List<SaturationExtractAnalysisExtractResponseDto> getSaturationExtractAnalysisExtractsByRange(Long rangeExtractId,
                                                                                                             String username) {
            UserModel owner = findUserByUsernameOrThrow(username);
            checkUserIsProprietario(owner);
            ...
            return saturationExtractAnalysisExtractRepository.findAllByRangeExtract(rangeExtract).stream()
        ...
        @Override
        @Transactional(readOnly = true)
        public List<SaturationExtractAnalysisExtractResponseDto> getSaturationExtractAnalysisExtractsByLayer(Long layerExtractId,
                                                                                                             String username) {
            UserModel owner = findUserByUsernameOrThrow(username);
            checkUserIsProprietario(owner);
            ...
            return saturationExtractAnalysisExtractRepository.findAllByLayerExtract(layerExtract).stream()
                    .map(SaturationExtractAnalysisExtractModel::toDto)
                    .collect(Collectors.toList());
        }
        ...
        @Override
        @Transactional
        public SaturationExtractAnalysisExtractResponseDto updateSaturationExtractAnalysisExtract(Long saturationExtractAnalysisExtractId,
            ...
            SaturationExtractAnalysisExtractModel updatedExtract = saturationExtractAnalysisExtractRepository.save(analysisExtract);
        ...
        private void checkUserIsProprietario(UserModel user) {
            if (user.getCargo() != Cargo.PROPRIETARIO) {
                throw new AccessDeniedException("Acesso negado. Apenas usuários com o cargo 'PROPRIETARIO' podem gerenciar extratos de análises.");
            }
        }
        ...
        private SaturationExtractAnalysisExtractModel findSaturationExtractAnalysisExtractByIdOrThrow(Long saturationExtractAnalysisExtractId) {
            return saturationExtractAnalysisExtractRepository.findById(saturationExtractAnalysisExtractId)
                    .orElseThrow(() -> new EntityNotFoundException("Extrato de análise de saturação não encontrado com o ID: " + saturationExtractAnalysisExtractId));
        }
