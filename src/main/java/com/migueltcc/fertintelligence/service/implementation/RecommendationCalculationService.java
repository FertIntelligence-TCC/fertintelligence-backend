...
public class RecommendationCalculationService {
    ...
    public RecommendationCalculationResult calculate(RecommendationCreateRequestDto dto, UserModel user, PropertyModel property, PlotModel plot) {
        ...
        diagnostics.add("Origem de adubos considerada: " + fertilizerSourceOption);
        ...
        if (tableOpt.isEmpty()) {
            ...
            FertilizerSelection planting = selectBestPlantingFertilizer(user, fertilizerSourceOption, requiredN, requiredP2O5, requiredK2O, warnings);
        ...
        return RecommendationCalculationResult.builder()
                ...
                .fertilizerSourceOption(fertilizerSourceOption.name())
...
return rows;}
    ...
    private FertilizerSourceOption resolveFertilizerSourceOption(FertilizerSourceOption option){return option==null?FertilizerSourceOption.BOTH:option;}
    private List<FormulatedMineralFertilizerModel> loadFormulatedBySource(UserModel user, FertilizerSourceOption source){
        return switch (source){
            case PRIVATE -> formulatedMineralFertilizerRepository.findAllByUser(user);
            case PUBLIC -> formulatedMineralFertilizerRepository.findAllByPublicoTrueOrderByIdAsc();
            case BOTH -> dedup(formulatedMineralFertilizerRepository.findAllByUser(user), formulatedMineralFertilizerRepository.findAllByPublicoTrueOrderByIdAsc(), FormulatedMineralFertilizerModel::getId);
        };
    }
    private List<SimpleMineralFertilizerModel> loadSimpleBySource(UserModel user, FertilizerSourceOption source){
        return switch (source){
            case PRIVATE -> simpleMineralFertilizerRepository.findAllByUser(user);
            case PUBLIC -> simpleMineralFertilizerRepository.findAllByPublicoTrueOrderByNameAsc();
            case BOTH -> dedup(simpleMineralFertilizerRepository.findAllByUser(user), simpleMineralFertilizerRepository.findAllByPublicoTrueOrderByNameAsc(), SimpleMineralFertilizerModel::getId);
        };
    }
    ...
    private Optional<SoilAnalysisModel> findLatestSoilFertilityAnalysis(PlotModel plot) {return soilAnalysisRepository.findTopByPlotOrderByIdDesc(plot);} private Optional<SaturationExtractAnalysisExtractModel> findLatestSaturationExtractAnalysis(PlotModel plot) {return saturationExtractAnalysisExtractRepository.findTopByRangeExtractAnalysisPlotOrderByIdDesc(plot).or(() -> saturationExtractAnalysisExtractRepository.findTopByLayerExtractAnalysisPlotOrderByIdDesc(plot));}
