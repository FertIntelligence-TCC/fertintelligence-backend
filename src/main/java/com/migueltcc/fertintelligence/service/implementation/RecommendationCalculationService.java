...
public class RecommendationCalculationService {
    ...
    public RecommendationCalculationResult calculate(RecommendationCreateRequestDto dto, UserModel user, PropertyModel property, PlotModel plot) {
        ...
        if (tableOpt.isEmpty()) {
            ...
            FertilizerSelection planting = selectBestPlantingFertilizer(user, requiredN, requiredP2O5, requiredK2O, warnings);
    ...
    private Optional<FertilityAnalysisExtractModel> findLatestFertilityExtract(SoilAnalysisModel soil){return fertilityAnalysisExtractRepository.findAll().stream().filter(e->(e.getRangeExtract()!=null&&e.getRangeExtract().getFertilityAnalysis()!=null&&Objects.equals(e.getRangeExtract().getFertilityAnalysis().getId(),soil.getId()))||(e.getLayerExtract()!=null&&e.getLayerExtract().getFertilityAnalysis()!=null&&Objects.equals(e.getLayerExtract().getFertilityAnalysis().getId(),soil.getId()))).max(Comparator.comparing(FertilityAnalysisExtractModel::getId));}
