...
public interface SimpleMineralFertilizerRepository extends JpaRepository<SimpleMineralFertilizerModel, Long> {
    ...
    @Query("select f from SimpleMineralFertilizerModel f where f.user.username = :username order by f.name asc")
    List<SimpleMineralFertilizerModel> findAllByUserUsernameOrderByNameAsc(@Param("username") String username);
    ...
    List<SimpleMineralFertilizerModel> findAllByPublicoTrueAndUser_CargoNotOrderByNameAsc(Cargo cargo);
