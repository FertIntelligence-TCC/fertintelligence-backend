...
public interface OrganicFertilizerRepository extends JpaRepository<OrganicFertilizerModel, Long> {
    List<OrganicFertilizerModel> findAllByUserAndPublicoFalseOrderByNameAsc(UserModel user);

    List<OrganicFertilizerModel> findAllByUser(UserModel user);

    List<OrganicFertilizerModel> findAllByUserOrderByNameAsc(UserModel user);

    @Query("select f from OrganicFertilizerModel f where f.user.username = :username order by f.name asc")
    List<OrganicFertilizerModel> findAllByUserUsernameOrderByNameAsc(@Param("username") String username);
